...
namespace ProgrammersBlog.MVC.Areas.Admin.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        public UserController(UserManager<User> userManager,
                              ...
                              IImageHelper imageHelper, IToastNotification toastNotification = null) : base(userManager, mapper, imageHelper)
        {
        ...
        public async Task<IActionResult> Add(UserAddDto userAddDto)
        {
            ...
            {
                var uploadedImageDto = await ImageHelper.Upload(userAddDto.UserName, userAddDto.PictureFile, PictureType.User);
                userAddDto.Picture = uploadedImageDto.ResultStatus == ResultStatus.Success ? uploadedImageDto.Data.FullName : "userImages/defaultUser.png";
                ...
                if (result.Succeeded)
                {
                    var userAddAjaxModel = JsonSerializer.Serialize(new UserAddAjaxViewModel
                    {
                        ...
                        {
                            ResultStatus = ResultStatus.Success,
                            Message = $"{user.UserName} user is added successfully.",
                            User = user
                        },
        ...
        public async Task<JsonResult> Delete(int userId)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var deletedUserDto = JsonSerializer.Serialize(new UserDto
                {
                    ResultStatus = ResultStatus.Success,
            ...
            else
            {
                ...
                var deletedErrorUserDto = JsonSerializer.Serialize(new UserDto
                {
                    ResultStatus = ResultStatus.Error,
                    Message = $"Error occured while deleting {user.UserName} user.\n{errorMessages}",
                    User = user
                });

                return Json(deletedErrorUserDto);
            }
        }
        ...
        public async Task<IActionResult> Update(UserUpdateDto userUpdateDto)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var uploadedImageDto = await ImageHelper.Upload(userUpdateDto.UserName, userUpdateDto.PictureFile, PictureType.User);
                    userUpdateDto.Picture = uploadedImageDto.ResultStatus == ResultStatus.Success ? uploadedImageDto.Data.FullName : "userImages/defaultUser.png";
                ...
                if (result.Succeeded)
                {
                    ...
                    var userUpdatedViewModel = JsonSerializer.Serialize(new UserUpdateAjaxViewModel
                    {
                        ...
                        {
                            ResultStatus = ResultStatus.Success,
                            Message = $"{updatedUser.UserName} user is updating successfully.",
                            User = updatedUser
                        }
                    });
            ...
            {
                var userUpdatedModelStateErrorViewModel = JsonSerializer.Serialize(new UserUpdateAjaxViewModel
                {
                    UserUpdateDto = userUpdateDto,
                    UserUpdatePartial = await this.RenderViewToStringAsync("_UserUpdatePartial", userUpdateDto),
                });

                return Json(userUpdatedModelStateErrorViewModel);
            }
        }
        [Authorize]
        [HttpGet]
        ...
        public async Task<ViewResult> ChangeDetails(UserUpdateDto userUpdateDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _toastNotification.AddSuccessToastMessage($"{updatedUser.UserName} user is updating successfully.");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (isVerified)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        _toastNotification.AddSuccessToastMessage("Password is updated successfully.");
                        return View();
                    }
                    else
                    {
